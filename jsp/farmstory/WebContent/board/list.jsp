...
<div id="board">
	...
	<nav class="paging">
				...
				<c:if test="${ groupStartEnd[0] > 1 }">
					<a href="/farmstory/board/view.do?pg=${ groupStartEnd[0]-1 }" class="prev">이전</a>
				</c:if>
				<c:forEach var="k" begin="${ groupStartEnd[0] }" end="${ groupStartEnd[1] }">
					<a href="/farmstory/board/view.do?pg=${k}" class="${k==pg ? 'current':''} num">${k}</a>
				</c:forEach>

				<c:if test="${ groupStartEnd[1] < pageEnd }">
					<a href="/farmstory/board/view.do?pg=${ pageEnd+1 }" class="next">다음</a>
				</c:if>
				</span>
	</nav>
